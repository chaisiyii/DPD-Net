...
from detectron2.detectron2.modeling.roi_heads import build_roi_heads
from detectron2.detectron2.modeling.meta_arch.build import META_ARCH_REGISTRY

__all__ = ["SingleCellRCNN"]


@META_ARCH_REGISTRY.register()
class SingleCellRCNN(nn.Module):
    """
    My modified Generalized R-CNN used for cell detection. 
    ...
    def inference(
        ...
        if detected_instances is None:
            ...
            roi_outs = self.roi_heads(images, features, proposals, None)
            if len(roi_outs) == 2:
                results, _ = roi_outs
            else:  # len=3
                results, box_features, _ = roi_outs
                del box_features
        ...
        if do_postprocess:
            ...
            return SingleCellRCNN._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
